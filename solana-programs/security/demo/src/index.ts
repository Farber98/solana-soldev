...
async function updateTestMovieReview(signer: web3.Keypair, programId: web3.PublicKey, connection: web3.Connection) {
    let buffer = Buffer.alloc(1000)
    const movieTitle = `Braveheart 3`
    movieInstructionLayout.encode(
        {
...
async function main() {
    ...
    const movieProgramId = new web3.PublicKey('3yah7hrSyDb1aK9JCoW9zKriHLCkaXmqDRvL8WG3QnSU')
    await sendTestMovieReview(signer, movieProgramId, connection)

    setTimeout(() => {
    }, 5000);

    await updateTestMovieReview(signer, movieProgramId, connection)
